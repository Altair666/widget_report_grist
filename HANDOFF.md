...
### v0.50.0 — fix: кнопка шестерёнки перенесена внутрь панели «Последние фильтры»

- Убрана position:fixed на кнопке ⚙ — она торчала за рамку панели #last-filters.
- Кнопка теперь в flex-строке заголовка панели (h3 + кнопка), без видимой рамки в покое.
- Стиль: прозрачный фон по умолчанию, hover даёт серый фон (#eceef0).
...
- fileAPI/download — кастомный путь без CORS-заголовков → fetch блокировался.
- Переключено на `{baseUrl}/attachments/{id}/download?auth={token}` — стандартный
  Grist API под /api/, тот же CORS что у SQL-эндпоинта, токен принимает.
- Результат: fetch → blob → <a download> работает без новых вкладок и диалогов.
- Фолбэк на window.open + fileAPI если и это не сработает.

### v0.47.1 — fix: fetch+blob вместо FSA/window.open + переименование кнопки

- FSA (`showDirectoryPicker`) заблокирован в cross-origin iframe Grist → убран.
- Новый подход: fetch(fileAPI?auth=token) → Blob → blob:// URL → <a download> click.
  Blob URL всегда same-origin → download атрибут работает → файл уходит в папку
  загрузок без диалогов и без новых вкладок.
- Если fetch заблокирован CORS — фолбэк на window.open (первый файл как тест).
- Кнопка переименована: "Скачать калибровочные коэффициенты", подпись убрана.

### v0.47.0 — feat: File System Access API для выгрузки CC-файлов в папку

- В Chrome/Edge: кнопка "Выгрузить коэффициенты" открывает системный диалог
  выбора папки (один раз), затем все файлы скачиваются через fetch() + FSA
  FileSystemFileHandle без лишних диалогов.
- Аутентификация fetch: auth-токен из getAccessToken() передаётся как ?auth=.
- Имя файла: из Content-Disposition заголовка (если сервер отдаёт), иначе
  `cc_{productCode}_{fileId}`.
- Если пользователь отменил пикер — выгрузка прерывается.
- Фолбэк (Firefox / Safari / CORS-блок): window.open() новые вкладки как раньше.

### v0.46.5 — fix: вернул оригинальный URL fileAPI/download/{id}

- URL возвращён к `https://db.mp-lab.ru/fileAPI/download/{id}` (без auth).
- getAccessToken убран из exportCalibrationFiles — не нужен для этого эндпоинта.
- target='_blank' сохранён (ключевое исправление из v0.46.4).
...
- Имя таблицы `product_files` → `Product_files` — все таблицы в этом документе
  с заглавной буквы; строчное имя давало [Sandbox] KeyError.

### v0.46.1 — fix: диагностические сообщения CC + обёртка getAccessToken

- `exportCalibrationFiles`: если CC-файлы не найдены — toast показывает кол-во строк
  и список колонок таблицы `product_files` (помогает диагностировать несовпадение схемы).
- `getAccessToken` обёрнут в отдельный try/catch → отдельное сообщение об ошибке токена.
- Добавлена проверка что tokenInfo.token не пустой.

### v0.46.0 — fix: CC-загрузка (fetchTable вместо SQL fetch) + сохранение выбора в фильтре

- CC-выгрузка: SQL-запрос через fetch() заменён на `grist.docApi.fetchTable(product_files)` —
  postMessage, без CORS; токен по-прежнему получается через `getAccessToken` для download-URL.
- Сохранение фильтра (Применить + Печать): в `batchValue` добавляется `selectedIds` —
  массив id выбранных продуктов (чекбоксы).
- Восстановление фильтра (`reapplyLastFilter`): после загрузки продуктов
  `state.selectedProductIds` заполняется из `snap.selectedIds`, чекбоксы восстанавливаются.

### v0.45.1 — fix: CC-загрузка через прямую ссылку + концентричные радиусы чипов

- CC-выгрузка: заменён fetch-blob на прямую <a href> ссылку — обходит CORS-ограничения
  iframe/виджета. Ошибки SQL теперь показываются текстом в toast.
- Чипы: border-radius 14px → 8px; padding 4px 4px 4px 4px (left) + 8px (right).
  Концентрично с аватаром: avatar 4px + gap 4px = chip 8px.
...
- btn-select-tpl: div → button, теперь disabled реально блокирует клик
- templatesForCurrentFilter: строгая фильтрация — только шаблоны с совпадающим
  basicPlatform; без БИ список пуст (пикер и так недоступен)
- onProductFilterChange(basicPlatform): если выбранный шаблон принадлежит другому
  БИ — сбрасывается автоматически

### v0.42.2 — fix: revert 0.42.1; кнопки шаблонов заблокированы без БИ

- Откат лишнего из v0.42.1 (авто-открытие пикера, кнопка Создать в пикере, CSS)
- «Выбрать шаблон» и «Создать шаблон» теперь disabled пока не выбрано базовое изделие
- Подпись «не выбран» → «выберите базовое изделие» когда БИ не задано

### v0.42.1 — feat: пикер шаблонов при выборе БИ; кнопка «Создать» внутри пикера

- При выборе базового изделия в фильтре — автоматически открывается пикер
  шаблонов, если текущий шаблон не подходит к новому БИ
- В шапке пикера показывается название выбранного БИ
- Кнопка «+ Создать шаблон» внутри пикера: закрывает модал и открывает редактор
  (новый шаблон автоматически получает basicPlatform из текущего фильтра — так было и раньше)

### v0.42.0 — feat: фильтр сохраняет дату/ОТК/шаблон; fix: пользователь с REST fallback

- **batchValue фильтра** (и при печати PDF, и при кнопке «Сохранить фильтр»):
  дополнен полями `date` (YYYY-MM-DD), `otkRole`, `otkPerson`, `templateId`
- **reapplyLastFilter**: при клике на чип восстанавливает дату (date-pick + display),
  ОТК-роль/ФИО (select + updateOtkDisplays), выбранный шаблон (selectedTemplate + renderTemplateButtons)
- **Пользователь**: `getUserProfile()` теперь с 5s timeout + fallback через
  `GET {apiBase}/profile/user` с Bearer-токеном из `getAccessToken()`; используется
  `profile.name || profile.email` чтобы не падать, если имя не заполнено

### v0.41.5 — fix: ШРИФТ абсолютно позиционирован; инпут 40px; align-items:center

### v0.41.4 — fix: drag-items align-items:flex-end — инпуты и кнопки по нижнему краю

### v0.41.3 — fix: инпут шрифта 28×28px (квадрат = rotate-btn)

### v0.41.2 — fix: инпут шрифта 28px; заголовок 11px; убрано «ОТК» из второго блока

### v0.41.1 — fix: компактные блоки шрифта; дата без текста в тулбаре

- **font-size-block**: высота 28px (= кнопки «↻»), ширина 28px для инпута; заголовок ШРИФТ/ШРИФТ ОТК — 8px, компактный
- **Дата в тулбаре**: текст «месяц год» скрыт (`display: none`); блок сжимается по ширине текста («Дата [1]»); `width: auto` переопределяет `--field-w`

### v0.41.0 — feat: фильтр по базовое+любой другой; refactor: блоки шрифта в одну строку

- **Загрузка изделий**: изменено условие загрузки — теперь требуется «Базовое изделие» + хотя бы один из: «Партия», «Исполнение», «Заказ» (ранее требовалось именно «Базовое + Партия»). SQL-запрос динамический: добавляет условия `part = ?`, `mode_name = ?`, `"order" = ?` только когда соответствующий фильтр выбран.
- **Исполнение/Заказ без продуктов**: пока базовое изделие выбрано, но второй фильтр ещё не выбран, выпадающие списки «Исполнение» и «Заказ» показывают весь каталог (раньше были пустыми).
- **Подсказка в таблице**: обновлён текст «Выберите базовое изделие и хотя бы ещё один фильтр».
- **Редактор шаблона**: блоки шрифта перенесены в одну строку — после «Дата» стоит блок «ШРИФТ» (заголовок над инпутом), затем «↻ ОТК», затем «ШРИФТ ОТК». Стиль заголовков шрифта идентичен заголовку раздела «КОНФИГУРАЦИЯ ИЗДЕЛИЯ».

### v0.40.0 — refactor: перестановка блоков в редакторе шаблона

- drag-col разбит на две строки: конфиг+серийный+дата+шрифт / отк+шрифт_отк (поля шрифтов теперь в config-block)
- group-row переведён на flex+justify-end с двумя .group-section (Активная группа / Кол-во групп) — выровнен вправо
- font-size-row удалён из name-block

### v0.39.0 — feat: настройки размера шрифта в шаблоне; fix: пользователь, фильтр печати

- **Размер шрифта PDF**: в редакторе шаблона добавлены два поля «Поля» и «ОТК» (диапазон 5–24пт, по умолчанию 10/9). Значения сохраняются в `Fields` JSON внутри Grist (формат `{fields:[...], fieldFontSize:N, otkFontSize:N}`). Обратная совместимость: если `Fields` — массив (старый формат), используются дефолты 10/9.
- **Пользователь**: `state.filterAuthor` теперь берётся из `grist.getUserProfile().name`; раньше было hardcoded «неизвестный пользователь».
- **Фильтр при печати**: `pushLastFilter` при нажатии «Печать» теперь сохраняет `batchValue` (текущие значения bp/part/modification/order) и `searchValue`. Раньше `batchValue` был пустым → клик по такому фильтру-чипу сбрасывал все дропдауны и показывал пустую таблицу.

### v0.38.0 — fix: OTK позиционирование — реальные размеры поля вместо hardcode

- `redrawPlacedFields()` теперь измеряет реальные CSS-размеры каждого placed-field через `getBoundingClientRect()` после добавления в DOM и сохраняет `f.w = el.width/page.width`, `f.h = el.height/page.height`.
- `bindPlacedFieldDrag` аналогично обновляет `f.w`/`f.h` после каждого перемещения.
- `stampTemplateFields` для OTK-полей использует `f.w * width` (реальная ширина блока) и `f.h * height` (реальная высота блока) вместо `--field-w/1.4` (ширина config-источника) и hardcoded `31.57pt`. Это устраняет смещение вбок (OTK шире config) и вниз (border учтён).
- Вертикальное центрирование текста: `INK_CORR = otkSize * (968-251)/2000 = 3.227pt` из реальных hhea-метрик Montserrat вместо canvas-измерения (которое для кириллицы возвращало неточные значения со системным шрифтом).
- Масштаб `scale = CSS_OUTER_H_px / otkOuterH_pt` выводится из реального соотношения пиксели/точки — не зависит от hardcoded 1.4.

### v0.37.0 — fix: перевёрнутый текст (порядок и наложение) в PDF
- `drawTextMixed` добавлен параметр `reversed`. При reversed=true (180°) каждый следующий прогон смещается влево (`x -= w`) вместо вправо. Без этого pdf-lib рисует 180°-текст справа налево, но x рос вправо → прогоны перекрывались и отображались в обратном порядке ("годмесяц" вместо "месяцгод").
- Все вызовы `drawTextMixed` для повёрнутого режима обновлены: rotOpt→`reversed=true`, stdOpt→`reversed=false`.

### v0.36.0 — feat: шрифты встроены в виджет; fix: одно PDF-окно
- Montserrat Latin (U+0000-00FF) + Cyrillic (U+0400-04FF) WOFF base64 встроены прямо в HTML (4 файла × ~15-26KB, ~82KB итого). Нет зависимости от CDN, шрифты не «отваливаются».
- `stampTemplateFields` принимает `fonts={regLat,regCyr,medLat,medCyr}`. Новые хелперы `textRuns/textWidthMixed/drawTextMixed` разбивают строки на Latin/Cyrillic прогоны — каждый рисуется правильным подмножеством.
- fix(double-pdf): убран `<a>.click()` fallback. В Grist-sandbox `window.open` возвращал `undefined` → `!w === true` → оба пути открывали вкладку. Теперь только `window.open(url, '_blank')`.
